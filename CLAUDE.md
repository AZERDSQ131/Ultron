...
- **Model**: NVIDIA API (Nemotron and other NVIDIA-hosted models, e.g. vision models for photo analysis) is the default provider; DeepSeek's own API (not NVIDIA-hosted) is now wired up as a second provider, switchable at runtime via `/provider` (CLI, web, Telegram — cycles nvidia↔deepseek, or takes an explicit `nvidia`/`deepseek` argument) alongside `/model` (now provider-aware: lists NVIDIA's live `/v1/models` catalog or DeepSeek's two fixed models — `deepseek-v4-flash`/`deepseek-v4-pro` — depending on which provider is active). `config.provider` (`LlmProvider`, `src/config.ts`) holds the active provider; each provider remembers its own last-picked model (`providerModels`, private to `config.ts`) so toggling back and forth restores rather than resets. `setActiveProvider`/`setActiveModel` (`src/config.ts`) are the only mutators — every interface routes model/provider switches through them instead of assigning `config.nemotronModel` directly, so the per-provider stash never drifts out of sync. `src/core/llm/models.ts` (`listAvailableModels`/`resolveModelContext`) is the single place that knows how to enumerate/enrich models per provider — shared by the local CLI, the web server's `/api/models`+`/api/provider` (which the remote CLI and Telegram both go through), and Telegram's own `/model`. `visionModel` (photo analysis) always stays on NVIDIA regardless of `config.provider` — DeepSeek has no vision equivalent wired up. `nvidiaApiKey` stays `required()` unconditionally for that reason; `deepseekApiKey` is only validated lazily (`createNemotronModel`, `src/core/llm/nemotron.ts`) when the DeepSeek provider is actually selected. DeepSeek's OpenAI-compatible API has no equivalent to NVIDIA NIM's `chat_template_kwargs` reasoning knob, so `thinkingMode` is a no-op on that provider for now.
