...
- **Remote CLI**: `src/interfaces/cli/remote.ts` (`pnpm remote` / `start:remote` / the `ultron` bin) is a *separate* entry point from the local CLI (`src/interfaces/cli/index.ts`) — it never imports `config.js`/`graph.js`, talks to a ULTRON web server purely over HTTP/SSE (`ULTRON_SERVER_URL`), and is what should run on the Mac. The local CLI is untouched and still the right thing to run directly on whichever machine hosts the graph (e.g. on the Jetson itself, or for local dev). Don't merge the two or make the local CLI depend on network calls — that was a deliberate scope decision to avoid risking the existing, tested local CLI in a large rewrite.
