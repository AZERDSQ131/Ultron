...
**Current status (2026-07-25):** Telegram support has been retired. The supported clients are the local CLI, remote CLI, web UI and native iOS app. They share the graph, SQLite state and HTTP/SSE contract; chat event origins are `cli` or `app`. The older Telegram notes below are historical implementation context only and are no longer part of the active architecture.
...
- **Chat projects** (iOS only so far): `src/core/memory/projects.ts` (`ProjectRegistry`) stores lightweight folders (`name`/`icon`/`color`) a chat can be filed under via a new nullable `chats.project_id` column — the chat's own row/thread_id is untouched, so moving it in or out of a project is a single UPDATE, not a data move. Deleting a project unassigns its chats rather than deleting them (same "metadata flag" spirit as chat archiving). Routes: `GET/POST /api/projects`, `PATCH/DELETE /api/projects/:id`, `PATCH /api/chats/:id/project`; `GET /api/chats` now also returns each chat's `projectId`. On mobile, `MenuView.swift` shows a "Projets" section above the conversation list (each project a folder row you can drag a chat onto via `.draggable`/`.dropDestination`) and a "Nouveau projet" row opening `ProjectEditorSheet.swift` (name, emoji grid, color swatches + a `ColorPicker` fallback, color round-tripped through `Color+Hex.swift` as a plain `"#RRGGBB"` string so it stays platform-agnostic). `ProjectView.swift` is the per-project detail screen: list its chats, take one back out, start a new chat directly inside the project, rename/re-icon/re-color, or delete the project. Chats already filed under a project no longer show up duplicated in the flat recent/folder lists. The conversation list itself also changed shape: only today/yesterday render flat at the top now, everything else is bucketed into two collapsible `DisclosureGroup` folders at the *bottom* — "Semaine dernière" (2-7 days) and "Ancien" (everything older, folding in what used to be a separate "Dernier mois" bucket) — fixing a real bug in the process: `MenuView`'s grouping and `ChatListRow`'s date label both used a bare `ISO8601DateFormatter()`, which silently fails to parse the server's millisecond-precision timestamps and was dumping every chat into "Ancien"/blank-date; both now use the fractional-seconds-aware `Date(serverTimestamp:)` from `Models.swift`.
...
- **iOS Live Activity / Dynamic Island** (`ios/ULTRONLiveActivity/`, `ios/Shared/ULTRONTaskActivityAttributes.swift`, `ios/ULTRON/Networking/LiveActivityManager.swift`, `src/core/memory/liveActivities.ts`): one activity per app-originated turn. Compact island = **nothing on the left while running** (a blue `ProgressView(timerInterval:)` ring was built there first and removed on the user's explicit request after seeing it on device — don't reintroduce an indicator there), green check on success / red cross on failure / orange raised hand awaiting approval; the elapsed counter on the right is what carries "still running", and it is blank once the turn lands. Long-press expands to the tail of the conversation. **The Dynamic Island only renders *active* activities** — `activity.end()` drops it out of the island instantly no matter which `dismissalPolicy` is passed (`.default`'s "lingers after ending" only covers the Lock Screen), which is why `finish()` delivers the terminal state as a plain `update()` and deliberately keeps the activity alive; `dismissSeenActivities()` ends it when the app next becomes active (and sweeps activities this process doesn't own, which are orphans nothing is driving any more), or a 4s timer does when the app was already on screen to see the outcome. Every update path guards on `finished` so a late stream event can't revive the counter over a green check. Three constraints drive the design and must not be "simplified" away. **(1)** ActivityKit ignores SwiftUI animations that repeat indefinitely, so an indeterminate `ProgressView` renders *frozen* — that ruled out a real spinner, and it is why anything time-based here has to be timer-driven (`Text(timerInterval:)` for the counter, which keeps ticking on its own while the app is suspended and nothing can update the activity). `ContentState` carries epoch `Double`s rather than `Date`s so the server can emit the same shape as plain JSON without matching a Codable date strategy. **(2)** The user's Apple Developer account is the **free tier**, which cannot sign the Push Notifications capability: `Activity.request(pushType: .token)` fails, `LiveActivityManager` falls back to a local-only activity, and the whole APNs path in `liveActivities.ts` stays dark. It is kept, complete and payload-aligned with `ContentState`, for the day the account goes paid — do not delete it as dead code. Because push is unavailable, the server keeps each turn's state in an **in-memory** map (not SQLite: it only describes work the current process is running, and the old design stored it on the `live_activities` push-token row, which never exists without a token — so nothing was recorded at all) and exposes it at `GET /api/live-activities/state`; `reconcile()` polls it when the app becomes active and `resolveAfterStreamFailure()` polls it when the SSE socket dies. That last one matters: `streamGraphTurn` deliberately does *not* abort an app-originated turn when the client closes, so a dead socket says nothing about the outcome and must never be reported as a failure. Accepted limitation: app suspended, the activity freezes on its last known state once the ~30s background task assertion expires, and only snaps to the truth when the app is active again.
...
- **Sub-agents** (`spawn_agent`, `src/core/tools/agents.ts`, re-added 2026-07-27 after being removed in `9886aa3`): a chat can delegate one self-contained task to a sub-agent that runs in **its own conversation** with the same tools and hands back a final report. `toolsNode` already runs a message's tool calls through `Promise.all`, so several `spawn_agent` calls in one reply run genuinely in parallel — that is the whole point, and the tool description says so explicitly because a model left alone calls it one at a time. Deliberately **not** the framework that was removed: no permanent agents with their own personas, no background runs, no cross-thread wake-ups, no run registry, no extra UI panels. The tool *awaits* its sub-agent, so the parent turn's lifecycle **is** the sub-agent's lifecycle — that single decision is what removes most of the old machinery. Sub-agent chats carry `parent_chat_id`/`subagent_task`, inherit the parent's security mode (delegated work must never be more permissive than the chat that asked for it), and get `<user_model>`/`<health_recent>` dropped from their prompt via `configurable.subAgent` since a spawned run has nobody to adapt its tone to. Depth capped at 2 (`MAX_SPAWN_DEPTH`). **Stop cascades**, by explicit requirement, via three independent paths on purpose: `/api/stop` and the CLI's SIGINT both call `abortSubAgents(chatId)` (which recurses through children so a grandchild isn't orphaned), and the tool also listens to the parent's own abort signal rather than trusting that it reaches nested tool calls. The result is prefixed with a `[ultron:subagent chat=<id>]` marker so clients can turn the tool call into a link to the conversation it created — it travels in the result *text*, not just the SSE event, because the same parsing has to work on replayed history, which is the case that actually matters; it only matches at the **start** of the result, so a model echoing it in prose can't fabricate a link elsewhere. Observation is read-only on both clients (web: `attachSubAgentLink`/`setObserveOnly`, the composer is replaced by a banner back to the parent; iOS: `SubAgentChatView` presented as a `.sheet` from `ChatView`, not pushed — deliberately a modal detour rather than a `navigationDestination`) — nothing is listening on a sub-agent's other end, so an input would silently do nothing. On the web sidebar, sub-agent chats stay listed with a 🤖 badge (originally so a chat reachable from nowhere couldn't happen — see the 2026-07-20 sidebar note); **on iOS this was flipped 2026-07-27**: `MenuView`'s `visibleChats` now excludes any chat with a non-nil `parentChatId`, since their only useful entry point is the tool-call sheet in the parent conversation and a mislabeled `"CLI"`-origin row duplicating that in the home screen's list was reported as confusing, not helpful. `SubAgentChatView` also polls every 1.5s while `isRunning` (`pollWhileRunning`) instead of loading once and waiting on pull-to-refresh, so the sheet actually tracks a running sub-agent live. Known cost: parallel sub-agents saturate the provider's concurrency limit fast — a real `ResourceExhausted: Worker local total request limit reached (97/48)` was hit while testing this on the current account.

**iOS spawn_agent rendering, redone 2026-07-27**: a real report — tapping the old "Observer" button (living below a `DisclosureGroup` in `ToolCallGroupView`) sometimes just collapsed the tool block instead of opening the sub-agent — plus the ask for a clean, always-visible widget instead of a generic tool call, led to pulling spawn_agent out of the generic tool-call pipeline entirely rather than patching the tap target. `SubAgentWidgetView` (own file) renders a spawn_agent call as its own card — icon, title, running/finished status, a single "Voir" button as the only tappable region — never inside a `ToolCallGroupView`/`DisclosureGroup`. The harder problem this surfaced: spawn_agent *awaits* its sub-agent's entire run before returning anything, so historically the client only ever learned the child's chat id (parsed from the `[ultron:subagent chat=...]` marker) once the whole run — which can take minutes — had already finished; there was no way to show a working button while it was still going. Fixed with two independent paths, since there's no way to rejoin a live SSE stream after reconnecting: (1) live — `agents.ts`'s `spawnAgent` calls a new `runConfig.configurable.onSubAgentStarted` callback the instant `chats.createSubAgent` returns (before the long `getSubGraph().invoke()` even starts), threaded through by `streamGraphTurn` (`server.ts`) as a new `subagent_started` SSE event; a callback via `configurable` rather than a module-level `EventEmitter` on purpose, so the event only reaches the one chat that's actually streaming, not every concurrent request. (2) reconnect/nested — `activeSubAgentIds(chatId)` (previously written but never called) now backs a `runningSubAgentsOf(chatId)` helper included in `GET /api/chats/:id/messages`'s response (`runningSubAgents: {chatId, title, task}[]`), which `ChatTimelineBuilder.mergeRunningSubAgents` turns into the same widget for cases with no live stream to listen on: reopening a chat mid-run (`ChatView.reconnectToRunningTurn`) and watching a sub-agent that itself spawned a grandchild (`SubAgentChatView.load`). `ChatTimelineItem.loadHistory` special-cases `spawn_agent` tool_call/tool_result pairs into a finished `.subAgent` item (parsing the marker + the `Sub-agent "<label>" finished.` title line) instead of the generic `toolGroup`, and the live `addToolCall`/`addToolResult` builder methods now skip `spawn_agent` by name so it's never double-rendered as both a generic pending tool chip and a widget.
- **`pnpm build` and the web frontend**: the build script must `rm -rf dist/interfaces/web/public` before copying. `cp -r src/…/public dist/…/public` copies *into* the destination once it exists, so before this was fixed (2026-07-27) every build after the very first wrote to a nested `dist/interfaces/web/public/public` that is never served — meaning `pnpm start:web` served a frontend frozen at the first-ever build while `pnpm web` (tsx, straight from `src/`) looked correct. Any "my web change isn't showing up in production" report should check this first.
- **`GET /api/memory/documents`** (`readPromptDocuments`, `graph.ts`): read-only MEMORY.md + SOUL.md with their mtimes, read **fresh from disk on every call** — the copies `graph.ts` captures at module load would show stale content, since MEMORY.md is edited by hand and by `memory_write`. Backs the iOS Memory screen (`MemoryView.swift`, redesigned 2026-07-27), which now shows the three layers of what ULTRON remembers in order of user control: MEMORY.md (curated), SOUL.md (personality), then the passive `user_model_observations` table charted by day and broken down by kind. The GET route is registered before the `DELETE /api/memory/:id` pattern but can't be shadowed by it, since that one is method-guarded.
...
- **Usage tracking**: `src/core/memory/usage.ts` (`UsageRegistry`, `usage_log` table, global, never purged) logs every LLM call ULTRON makes — not just main chat turns but every cheap separate call too (`narrator.ts`'s two functions, `goalJudge.ts`, `userModelExtractor.ts`, `visionAnalyzer.ts`) — via `recordUsage()` in `src/core/llm/usage.ts`, called at each of those call sites right where real token usage (`response.usage_metadata`) is already available, tagged with a `kind` (`chat`/`narrator`/`goal_judge`/`user_model`/`vision`) and the active `config.provider`/model at call time. `recordUsage` takes an explicit provider override for `visionAnalyzer.ts` specifically, since vision always runs on NVIDIA regardless of `config.provider`. Never throws — a broken usage write must not break the actual reply. The three main-turn call sites (CLI's `executeTurn`, web's `streamGraphTurn`, Telegram's `runSingleTurn`) now compute `inputTokens`/`generatedTokens` unconditionally instead of only when `/verbose` is on, so usage is tracked regardless of that display setting. `GET /api/usage/summary?days=N` (`N=0` for all-time) returns `totals` plus `byProvider`/`byModel`/`byKind`/`byDay` breakdowns and the last 50 `recent` calls; every aggregate also carries an `elapsedMs` sum, which is what makes average latency per model/provider/kind computable client-side. It backs two surfaces: the web UI's "Tokens" view (`public/js/usageView.js`, `/tokens` command, sidebar button below Health) and the iOS `TokensView.swift`. The mobile screen (rebuilt 2026-07-27, Swift Charts) is the detailed one — stat tiles, a per-day stacked bar chart switchable between tokens/calls/cost, an input-vs-output split, a breakdown switchable between model/provider/kind with share bars, and a recent-calls log; before that it decoded only `totals` and `byKind` and silently dropped every other field the endpoint already returned. No CLI surface for this.
- **Server timestamps and `ISO8601DateFormatter`**: the server stamps rows with `new Date().toISOString()`, which includes milliseconds. A default-configured `ISO8601DateFormatter` is `.withInternetDateTime` only and returns **nil** on those, silently — use `Date(serverTimestamp:)` (`ios/ULTRON/Networking/Models.swift`), which opts into `.withFractionalSeconds` and falls back. `SchedulesView.swift`, `ChatListRow.swift` and `MenuView.swift` still construct bare `ISO8601DateFormatter()`s and so still swallow their dates; not fixed yet because it was outside the change that found it.
- **Finance module** (manual entry only, deliberately optimized for "just say it in chat" — bank sync was explicitly scoped out: a real DSP2/Enable Banking connector needs a developer Application ID, an RSA keypair, and a public OAuth redirect URI the Jetson doesn't have, too much setup per the user): `src/core/memory/finance.ts` (`FinanceRegistry`, global, never purged) stores `finance_accounts` (name/type/currency), `finance_balance_snapshots` (one upserted row per account per calendar day), and `finance_transactions` (signed amount, free-text category), plus analytics read paths — `getSpendingByCategory`, `getMonthlyCashFlow`, `currentMonthSummary` — computed on read, not stored. `getOrCreateAccount` (used by every write path, `DEFAULT_ACCOUNT_NAME = "Principal"` when the user names no account) is what makes logging a balance or transaction never require a separate "create account" step first — the opposite tradeoff from `findAccountByName`'s exact-match-only lookup elsewhere, which stays strict for reads/deletes so the model can't silently write to the wrong account once more than one exists. `getNetWorthHistory` forward-fills each account's last known balance across the requested range so a freshly-added account with no history doesn't make earlier days look like net worth dropped. `finance_add_account`/`finance_record_balance`/`finance_add_transaction`/`finance_query` (`src/core/tools/finance.ts`) are the conversational surface — `finance_record_balance`/`finance_add_transaction` are written to be called proactively (same pattern as `log_meal_or_exercise`: "call this whenever the user mentions...", not just when explicitly asked), with the model guessing a transaction's category itself. `GET /api/finance/summary?days=N` (accounts, net worth history, this-month summary, spending by category, 6-month cash flow, recent transactions) + `POST/DELETE /api/finance/accounts[/:id]` + `POST /api/finance/accounts/:id/{balance,transactions}` back the web UI's "Finance" view (`public/js/financeView.js`, `/finance` command, sidebar button below Tokens) — inline forms directly in the account cards (no modal), primarily a read dashboard (net worth trend, income vs. expenses chart, category breakdown) since chat is the primary way of logging anything now. `public/js/viewSwitcher.js` (`closeOtherViews`) is the shared registry the three swapped-in views (Health/Tokens/Finance) use to stay mutually exclusive — added when Finance made the previous pairwise "each view reaches into the next view's DOM directly" pattern from `healthView.js`/`usageView.js` not scale past two. The same endpoint backs iOS `FinanceView.swift` (redesigned 2026-07-27, Swift Charts): net-worth area chart over a selectable range, grouped income/expenses bars over 6 months, account rows with the date each balance was recorded, category shares and a transaction log. Two traps that surface there — `spendingByCategory.total` is `SUM(amount)` over negative rows so it arrives **negative** and has to be rendered as a magnitude, while `monthlyCashFlow`/`currentMonthSummary` already return `expenses` as positive magnitudes; and `finance_transactions.id` is an INTEGER, which the iOS model had typed as `String`, so the whole `FinanceSummary` decode would have failed the moment one transaction existed. No CLI surface for this.

- **Deep Research task mode** (`deep_research`, a fifth `TaskMode` in `graph.ts` alongside `none`/`todo`/`plan`/`goal`): the structure was taken from what published deep-research systems converge on (OpenAI/Gemini/Claude and the agent architectures written up around them), researched at the user's explicit request rather than invented — a **Plan → Search → Read → Reflect → Iterate → Synthesise** loop where the plan is a set of *sub-questions* (not actions), each round records findings with citations plus what it failed to resolve, and those recorded gaps drive the next round instead of the run stopping at the first plausible answer. Those systems fan out to parallel researcher sub-agents under a lead agent; ULTRON's sub-agent interface is retired, so the fan-out collapses to one sequential agent working the plan aspect by aspect. **What replaces the missing parallelism is `ResearchRegistry`** (`src/core/memory/research.ts`, per-chat `research_notes`, one row per note — unlike `TodoRegistry`'s single JSON blob, since notes are appended throughout a run and read in aggregate): a long run reads far more than fits in context and its own tool output is exactly what gets truncated and evicted as the turn grows, so without durable notes the report would silently get *worse* the more research was done. `research_note` writes one finding + source + optional `gap` as it's read; `research_review` hands them all back at synthesis time. The plan itself reuses the existing to-do list, so aspects appear in the web to-do panel for free. Notes are cleared at the same turn boundary the to-do list is (`server.ts` `handleTurn`, CLI's `/task` handler and turn boundary) or a new question would synthesise over the previous one's findings. `taskModeReminder` has its own `deep_research` branch because the stage to nudge toward depends on how many notes/gaps exist, which `TodoState` can't tell. Surfaces: web task menu + `/task research`, CLI/remote-CLI `/task research` (shorthand — the wire value stays `deep_research`), and iOS `TaskModePickerSheet` (which gained per-mode descriptions, since "Deep Research" isn't self-explanatory from a label).
