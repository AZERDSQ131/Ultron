...
- **Computer use**: `/computer-use <instruction>` (CLI-only, `src/core/computerUse.ts`) drives the real macOS UI through the Accessibility API (AXUIElement) — not screenshots/pixel-clicking. `native/computer-use/main.swift` is a small standalone Swift CLI (compiled lazily on first use by `ensureHelperBuilt()` in `src/core/tools/computer.ts`, binary gitignored) that dumps the frontmost app's UI as a structured tree (role/title/value/frame, addressed by a child-index path) and executes actions against it — `AXUIElementPerformAction(.press)` preferred, falling back to a physical `CGEvent` click at the element's real frame center. The loop: read tree as text → model picks one tool call (`computer_click_element`, `computer_type`, `computer_key`, `computer_open_app`, `computer_scroll_element`, `computer_wait`, `computer_finish`/`computer_fail`) → execute → re-read tree → repeat. `computer_open_app` shells out to `open -a <name>` — added after the original pixel-based version tried to type an app name directly with nothing focused to receive it, and the keystrokes landed on whatever window (including ULTRON's own terminal) happened to be focused. Since this reads structured text rather than images, it doesn't need a vision-capable model — `COMPUTER_USE_MODEL` (separate from the main chat model) defaults to `qwen/qwen3-next-80b-a3b-instruct`, picked from a live multi-turn bake-off (same induced-failure scenario: computer_open_app errors, does the model recover sensibly on the next turn) run across the user's original choice plus five candidates found free on OpenRouter and confirmed also served directly on this NVIDIA endpoint (no second provider needed): `deepseek-ai/deepseek-v4-flash` reliably degenerates on the *second* tool-calling turn under `tool_choice: "required"` — once its own prior tool call plus a `ToolMessage` are in history, it echoes the JSON Schema's type name (`"string"`) as the literal argument value instead of a real one, reproduced even pushing the API's own unmodified `AIMessage` (not a bug on ULTRON's side); `meta/llama-3.2-90b-vision-instruct` avoids that specific bug but wasn't the best of the wider field; `nvidia/nemotron-3-super-120b-a12b` hallucinated success without checking; `openai/gpt-oss-20b` leaked raw special tokens into the tool name; `qwen/qwen3.5-122b-a10b` went silent (no tool call) on the retry turn. `qwen3-next-80b-a3b-instruct` was the only one that recognized the failure and called `computer_fail` with an accurate reason instead of guessing, hallucinating, or stalling — picked for judgment, not just clean syntax. `nvidia/nemotron-3-ultra-550b-a55b` is a solid second choice (clean values throughout, just not self-correcting). Requires Accessibility permission granted to the compiled helper binary (`native/computer-use/bin/computer-use-helper`) in System Settings > Privacy & Security > Accessibility — `AXIsProcessTrusted()` is checked before any tree/click/scroll call and fails with an instructive message naming the binary path if not granted.
