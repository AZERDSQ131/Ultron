...
- **Computer use**: `/computer-use <instruction>` (CLI-only, `src/core/computerUse.ts`) drives the real macOS mouse/keyboard/screen via `@nut-tree-fork/nut-js` + `sharp` (`src/core/tools/computer.ts`) — screenshot → vision model picks one tool call → action → repeat, same shape as Anthropic's computer-use reference loop, but hand-rolled since none of these models have a built-in `computer` tool type. Uses a separate model from the main chat model (`COMPUTER_USE_MODEL`), because the main model isn't picked for vision. Every run first calls `verifyVisionSupport()`, a live probe against the NVIDIA endpoint (a 1x1 PNG + trivial prompt) — the command refuses to start if the served model rejects image input, rather than trusting the model id's name. This isn't a theoretical gate: DeepSeek V4 Flash was the original choice, but the live probe caught NVIDIA's hosted instance 500ing on image content (vLLM deployed without `--enable-multimodal`) — default switched to `meta/llama-3.2-90b-vision-instruct`, confirmed working with the same probe (`nvidia/nemotron-3-nano-omni-30b-a3b-reasoning` also confirmed working, smaller but reasons over what it sees). Revert `COMPUTER_USE_MODEL` to `deepseek-ai/deepseek-v4-flash` once NVIDIA flips that flag. No AXUIElement/accessibility-tree targeting yet (pixel-coordinate clicking only) — deliberately deferred, see the project's computer-use research for why that'd be a separate Swift-binary integration.
