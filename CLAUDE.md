...
- **Computer use**: `/computer-use <instruction>` (CLI-only, `src/core/computerUse.ts`) drives the real macOS mouse/keyboard/screen via `@nut-tree-fork/nut-js` + `sharp` (`src/core/tools/computer.ts`) — screenshot → vision model picks one tool call → action → repeat, same shape as Anthropic's computer-use reference loop, but hand-rolled since DeepSeek has no built-in `computer` tool type. Uses a separate model from the main chat model (`COMPUTER_USE_MODEL`, default `deepseek-ai/deepseek-v4-flash`), because the main model isn't picked for vision. Every run first calls `verifyVisionSupport()`, a live probe against the NVIDIA endpoint (a 1x1 PNG + trivial prompt) — the command refuses to start if the served model rejects image input, rather than trusting the model id's name. This isn't a theoretical gate: verified live during implementation, NVIDIA's hosted `deepseek-ai/deepseek-v4-flash` instance currently runs vLLM without `--enable-multimodal`, so it 500s on image content and `/computer-use` correctly blocks — `nvidia/nemotron-nano-12b-v2-vl` was confirmed working on the same endpoint as a fallback if a working default is needed before NVIDIA flips that flag. No AXUIElement/accessibility-tree targeting yet (pixel-coordinate clicking only) — deliberately deferred, see the project's computer-use research for why that'd be a separate Swift-binary integration.
