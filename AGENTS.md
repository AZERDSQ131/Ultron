...
- **Scheduled tasks**: persistent five-field cron tasks live in `src/core/memory/schedules.ts`, are available through `schedule_task` and `/api/schedules`, and are managed from the iOS Crons screen. The Agent/sub-agent interface is retired.
