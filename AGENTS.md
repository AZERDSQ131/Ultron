...
- **Chats**: not a single hardcoded thread anymore. `src/core/memory/chats.ts` (`ChatRegistry`) tracks every chat (id, title, timestamps) in the same database file — a chat's `id` doubles as its LangGraph `thread_id`. The web UI has a sidebar to list/create/rename/delete chats. The CLI tracks one "current chat" per process and resumes whichever chat was most recently active (from either interface) at startup; `/archive` finalizes the current chat and starts a new one instead of exiting, so an archived CLI chat stays reachable from the web sidebar. The pre-chats hardcoded thread id (`"ultron-main"`, exported as `LEGACY_CHAT_ID`) is migrated into the registry on startup via `chats.ensure(...)`.
